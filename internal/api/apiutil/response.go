// Package apiutil provides shared HTTP helpers for the api layer.
// It is kept in a separate package to avoid import cycles between the
// top-level api package (router) and the handlers sub-package.
package apiutil
...
// HandleError maps well-known domain errors to HTTP status codes and writes
...
// ParseUUID parses a UUID string, returning an error if the value is not valid.
func ParseUUID(s string) (uuid.UUID, error) {
	return uuid.Parse(s)
}

// ParseIntQuery reads an integer query parameter from r, returning def when
// the parameter is absent or cannot be parsed.
